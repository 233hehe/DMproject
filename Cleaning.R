...
c5<-c3 %>%
  select(Company.Name,Headquarters.Location,Category.Groups,Founded.Date,Closed.Date,Company.Length,
         Number.of.Employees,Number.of.Founders, Number.of.Articles,Number.of.Investors,Number.of.Lead.Investors,
         Number.of.Funding.Rounds,Last.Funding.Date,Last.Funding.Type,Last.Funding.Amount,Last.Equity.Funding.Amount,
...
c5$Category.Groups<-str_split(c3$Categories,",",simplify = TRUE)[,1]
c5$Category<-str_split(c3$Categories,",",simplify = TRUE)[,1]
#After we create more useful column we drop the useless now!!!
c6<-data.frame(cbind(c5,c4[,27:1299]))
...
c6$Number.of.Founders<-median(na.omit(c6$Number.of.Founders))
